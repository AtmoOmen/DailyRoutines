...
public unsafe class AutoNoviceNetwork : DailyModuleBase
{
    ...
    [Signature("48 89 5C 24 ?? 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 84 24 ?? ?? ?? ?? 48 8B D9 48 8B 49 ?? 48 8B 01 FF 90 ?? ?? ?? ?? 48 8B 0D ?? ?? ?? ?? 48 8B F8 E8 ?? ?? ?? ?? 48 85 C0 74 ?? 48 8B C8 E8 ?? ?? ?? ?? EB ?? 48 8B 87 ?? ?? ?? ?? 48 C1 E8 ?? 24 ?? 84 C0 75 ?? 48 8D 0D")]
